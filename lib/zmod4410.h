...
typedef struct {
  ...
  int8_t read; /**< function pointer to i2c read */
  int8_t write; /**< function pointer to i2c write */
...
/*int8_t arduino_i2c_read(uint8_t i2c_addr, uint8_t reg_addr, uint8_t *buf, uint8_t len)
  {
  typedef int8_t (*zmod44xx_i2c_ptr_t)(i2c_addr, reg_addr, data, len);
  }*/
...
int8_t zmod44xx_read_sensor_info(zmod44xx_dev_t *dev)
{
  ...
  do {
    ret = arduino_i2c_write(dev->i2c_addr, ZMOD44XX_ADDR_CMD, &cmd, 1);
    if (ret) {
      return ERROR_I2C;
    }
  ...
  ret = arduino_i2c_read(dev->i2c_addr, ZMOD44XX_ADDR_PID, data, ZMOD44XX_LEN_PID);
  if (ret) {
    return ERROR_I2C;
  }
  dev->pid = data[0] << 8 | data[1];

  ret = arduino_i2c_read(dev->i2c_addr, ZMOD44XX_ADDR_GENERAL_PURPOSE,
                  dev->general_purpose, ZMOD44XX_LEN_GENERAL_PURPOSE);
  if (ret) {
    return ERROR_I2C;
  }

  ret = arduino_i2c_read(dev->i2c_addr, ZMOD44XX_ADDR_CONF, dev->config,
                  ZMOD44XX_LEN_CONF);
  if (ret) {
    return ERROR_I2C;
  }
...
int8_t zmod44xx_init_sensor(zmod44xx_dev_t *dev)
{
  ...
  ret = arduino_i2c_read(dev->i2c_addr, 0xB7, data, 1);
  if (ret) {
    return ERROR_I2C;
  }
  ...
  ret = arduino_i2c_write(dev->i2c_addr, dev->init_conf->h.addr, data,
                   dev->init_conf->h.len);
  if (ret) {
    return ERROR_I2C;
  }

  ret = arduino_i2c_write(dev->i2c_addr, dev->init_conf->d.addr,
                   dev->init_conf->d.data, dev->init_conf->d.len);
  if (ret) {
    return ERROR_I2C;
  }

  ret = arduino_i2c_write(dev->i2c_addr, dev->init_conf->m.addr,
                   dev->init_conf->m.data, dev->init_conf->m.len);
  if (ret) {
    return ERROR_I2C;
  }

  ret = arduino_i2c_write(dev->i2c_addr, dev->init_conf->s.addr,
                   dev->init_conf->s.data, dev->init_conf->s.len);
  if (ret) {
    return ERROR_I2C;
  }

  ret = arduino_i2c_write(dev->i2c_addr, ZMOD44XX_ADDR_CMD, &dev->init_conf->start, 1);
  if (ret) {
    return ERROR_I2C;
  }
  ...
  ret = arduino_i2c_read(dev->i2c_addr, dev->init_conf->r.addr, data,
                  dev->init_conf->r.len);
  if (ret) {
    return ERROR_I2C;
  }
...
int8_t zmod44xx_init_measurement(zmod44xx_dev_t *dev)
{
  ...
  ret = arduino_i2c_write(dev->i2c_addr, dev->meas_conf->h.addr, data,
                   dev->meas_conf->h.len);
  if (ret) {
    return ERROR_I2C;
  }

  ret = arduino_i2c_write(dev->i2c_addr, dev->meas_conf->d.addr,
                   dev->meas_conf->d.data, dev->meas_conf->d.len);
  if (ret) {
    return ERROR_I2C;
  }

  ret = arduino_i2c_write(dev->i2c_addr, dev->meas_conf->m.addr,
                   dev->meas_conf->m.data, dev->meas_conf->m.len);
  if (ret) {
    return ERROR_I2C;
  }

  ret = arduino_i2c_write(dev->i2c_addr, dev->meas_conf->s.addr,
...
int8_t zmod44xx_start_measurement(zmod44xx_dev_t *dev)
{
  ...
  ret = arduino_i2c_write(dev->i2c_addr, ZMOD44XX_ADDR_CMD, &dev->meas_conf->start, 1);
  if (ret) {
    return ERROR_I2C;
  }
  return ZMOD44XX_OK;
...
int8_t zmod44xx_read_status(zmod44xx_dev_t *dev, uint8_t *status)
{
  ...
  ret = arduino_i2c_read(dev->i2c_addr, ZMOD44XX_ADDR_STATUS, &st, 1);
  if (0 != ret) {
    return ret;
  }
  *status = st;
...
int8_t zmod44xx_read_adc_results(zmod44xx_dev_t *dev, uint8_t *adc_result)
{
  ...
  ret = arduino_i2c_read(dev->i2c_addr, dev->meas_conf->r.addr, adc_result,
                  dev->meas_conf->r.len);
  if (ret) {
    return ERROR_I2C;
  }
